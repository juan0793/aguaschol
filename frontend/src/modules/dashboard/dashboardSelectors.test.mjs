...
import { clampSplit, interestPerCapital, ofEachTen, SPLIT_DEFAULT, debtRanking, filterBarriosByQuery, formatCompactCurrency, lastDaysSeries, metricValueOf, normalizeBarrioText, selectedRankedRows, sumSelectedDebt, sumSelectedServices } from "./dashboardSelectors.js";
...
// Lecturas del resumen de cartera.
assert.equal(ofEachTen(12925, 19106), "7 de cada 10");
assert.equal(ofEachTen(1, 0), "");
assert.equal(Math.round(interestPerCapital(116561243.32, 115357814.1) * 100) / 100, 1.01);
assert.equal(interestPerCapital(5, 0), 0);
