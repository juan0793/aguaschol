...
const FLOW_STEPS = [
  { state: "pending", label: "Preparar", title: "Completar datos y programar la visita", action: "Iniciar visita" },
  { state: "visit", label: "Visita", title: "Registrar los hallazgos de campo", action: "Confirmar hallazgo" },
  { state: "confirmed", label: "Confirmar", title: "Revisar la ficha y preparar el aviso", action: "Iniciar regularización" },
  { state: "regularization", label: "Seguimiento", title: "Dar seguimiento al aviso entregado", action: "Marcar regularizada" },
  { state: "regularized", label: "Cierre", title: "Expediente regularizado y finalizado", action: "" }
];
...
export default function FichaDrawer({ record, api, config, onClose, onSaved, onPrintFicha, onPrintAviso, notify }) {
  ...
  const currentStep = FLOW_STEPS.findIndex((item) => item.state === form.estado_operativo);
  const nextState = transitions.find((state) => FLOW_STEPS.findIndex((item) => item.state === state) > currentStep);
  const previousState = transitions.find((state) => state !== "discarded" && FLOW_STEPS.findIndex((item) => item.state === state) < currentStep);
  const stepCopy = FLOW_STEPS[Math.max(0, currentStep)];
  ...
  return createPortal(<><div className="cl-drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="cl-drawer" role="dialog" aria-modal="true" aria-labelledby="cl-drawer-title">
    ...
    {form.id ? <div className="cl-progress" aria-label="Progreso de la ficha">{FLOW_STEPS.map((item, index) => <span key={item.state} className={index <= currentStep ? "is-done" : ""}>{index + 1}<small>{item.label}</small></span>)}</div> : null}
    <form onSubmit={save}>
      {form.id && stepCopy ? <section className={`cl-next-step ${nextState ? "" : "is-complete"}`} aria-label="Siguiente acción recomendada"><div><span>{nextState ? "Siguiente acción" : "Proceso completado"}</span><strong>{stepCopy.title}</strong><small>{nextState ? "Guarda los cambios de esta etapa antes de continuar." : "Puedes consultar, imprimir o revisar el historial de la ficha."}</small></div><div>{previousState ? <button type="button" className="cl-quiet" disabled={saving} onClick={() => setState(previousState)}>Volver a {LABELS[previousState]}</button> : null}{nextState ? <button type="button" className="cl-primary" disabled={saving || (["confirmed","regularization","regularized"].includes(nextState) && !config.permissions.can_confirm_or_regularize)} onClick={() => setState(nextState)}>{stepCopy.action}<Icon name="arrowRight" /></button> : null}</div></section> : null}
      <div className="cl-drawer-scroll"><fieldset className="cl-drawer-fieldset" disabled={!config.permissions.can_manage_ficha_state || saving}><FichaSections form={form} onChange={change} onValidatePadron={validatePadron} history={history} canEditInternal={config.permissions.can_manage_ficha_state} /></fieldset></div>
      <footer><div>{form.id && config.permissions.can_manage_ficha_state && transitions.includes("discarded") ? <details className="cl-discard"><summary>Descartar ficha</summary><label className="cl-reason"><span>Motivo obligatorio</span><input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explica por qué se descarta" /></label><button type="button" className="cl-danger" disabled={saving || !reason.trim() || !config.permissions.can_confirm_or_regularize} onClick={() => setState("discarded")}>Confirmar descarte</button></details> : null}</div><div className="cl-drawer-main-actions"><button type="button" className="cl-secondary" onClick={() => onPrintFicha(form)}><Icon name="print" />Ficha técnica</button><button type="button" className="cl-secondary" onClick={() => setAvisoOpen(true)}><Icon name="records" />Editar aviso</button>{config.permissions.can_manage_ficha_state ? <button type="submit" className="cl-primary" disabled={saving}><Icon name="success" />{saving ? "Guardando…" : "Guardar ficha"}</button> : null}</div></footer>
    </form>
  </aside></div>{avisoOpen ? <AvisoEditor record={form} onClose={() => setAvisoOpen(false)} onPreview={(draft) => { setForm(draft); setAvisoOpen(false); onPrintAviso(draft); }} /> : null}</>, document.body);
}
