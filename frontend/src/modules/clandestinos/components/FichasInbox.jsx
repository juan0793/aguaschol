...
export default function FichasInbox({ model, selectedIds, onToggle, onToggleVisible, onSelectAll, onClearSelection, onCompare, onPrintSummary, comparison, bulkLoading, onOpen, onNew, canCreate = false }) {
  ...
  return <section className="cl-inbox" aria-label="Bandeja de trabajo de fichas">
    ...
    <div className="cl-indicators" role="group" aria-label="Filtrar por etapa">{[["draft","Borradores","edit"],["pending","Por visitar","calendar"],["visit","En visita","map"],["confirmed","Aviso pendiente","mail"],["regularization","En seguimiento","history"],["regularized","Cerradas","success"],["discarded","Descartadas","archive"]].map(([key,label,icon]) => <button type="button" key={key} aria-pressed={model.filters.state === key} className={model.filters.state === key ? "is-active" : ""} onClick={() => { setAlertsOnly(false); model.filters.setState(model.filters.state === key ? "" : key); }}><Icon name={icon} /><span>{label}</span><strong>{model.counts[key] || 0}</strong></button>)}</div>
