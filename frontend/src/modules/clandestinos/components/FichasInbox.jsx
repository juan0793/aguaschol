...
export default function FichasInbox({ model, selectedIds, onToggle, onToggleVisible, onSelectAll, onClearSelection, onCompare, onPrintSummary, comparison, bulkLoading, onOpen, onNew, canCreate = false }) {
  ...
  return <section className="cl-inbox" aria-label="Bandeja de trabajo de fichas">
    ...
    <div className="cl-indicators">{[["draft","Borradores"],["pending","Por visitar"],["visit","En visita"],["confirmed","Aviso pendiente"],["regularization","En seguimiento"],["regularized","Cerradas"],["discarded","Descartadas"]].map(([key,label]) => <button type="button" key={key} className={`${model.filters.state === key ? "is-active" : ""} ${key === "draft" || key === "discarded" ? "is-side" : ""}`.trim()} onClick={() => { setAlertsOnly(false); model.filters.setState(model.filters.state === key ? "" : key); }}><span>{label}</span><strong>{model.counts[key] || 0}</strong></button>)}</div>
    <section className="cl-service-stats" aria-label="Conexiones de servicios registradas">
      <header className="cl-service-stats-head"><div><span className="cl-kicker">Lectura de servicios</span><h3>¿Qué servicios tienen los clandestinos?</h3><p>{serviceTotal ? `Sobre ${serviceTotal} fichas con los filtros actuales.` : "Las cifras aparecerán al cargar fichas."}</p></div><span className="cl-service-stats-note">Agua potable · Aguas negras / residuales</span></header>
      <div className="cl-service-stats-grid">{serviceCards.map(([key, label, icon, tone]) => { const value = Number(serviceStats[key] || 0); const percent = serviceTotal ? Math.round((value / serviceTotal) * 100) : 0; return <article className={`cl-service-stat ${tone}`} key={key}><div className="cl-service-stat-top"><span className="cl-service-stat-icon"><Icon name={icon} /></span><span>{label}</span></div><strong>{value}</strong><div className="cl-service-stat-meter"><i style={{ width: `${percent}%` }} /></div><small>{percent}% del resultado</small></article>; })}</div>
    </section>
    ...
    </tbody></table>{model.refreshing ? <span className="cl-table-refresh" role="status">Actualizando…</span> : null}</div>
