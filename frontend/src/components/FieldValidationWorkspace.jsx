...
const FieldValidationWorkspace = ({ apiFetch, apiUrl, barrioCodes = [], isActive, focusRequest, onFocusConsumed, onOpenSig }) => {
  ...
  useEffect(() => {
    if (!focusRequest?.pointId || !historyPoints.length) return;
    if (historyPoints.some((point) => point.id === focusRequest.pointId)) {
      editPoint(focusRequest.pointId);
      onFocusConsumed?.();
    }
  }, [focusRequest, historyPoints, editPoint, onFocusConsumed]);
