...
// Boton de confirmacion por presion sostenida. Sustituye a window.confirm en
// acciones irreversibles: el gesto es deliberado y no interrumpe con un modal.
// Adaptado de React Bits (categoria Micro) a los tokens --ds-* del proyecto.
...
export default function HoldButton({
  children = "Mantener para eliminar",
  doneLabel = "Confirmado",
  icon = null,
  doneIcon = null,
  backgroundColor = "var(--ds-danger-soft, #fdeaec)",
  fillColor = "var(--ds-danger, #9b202d)",
  textColor = "var(--ds-danger, #9b202d)",
  ...
  const endPointer = (event, options) => {
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(currentTimers.complete);
  ...
  return (
    ...
    >
      ...
      <span id={hintId} className="hold-button__sr">
        Manten presionado {seconds} segundos para confirmar
      </span>
    </button>
  );
}
