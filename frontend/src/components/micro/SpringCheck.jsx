...
// Casilla de seleccion con respuesta elastica: el relleno y el tick confirman
// el cambio en listados densos donde el checkbox nativo pasa desapercibido.
// Adaptado de React Bits (categoria Micro): icono propio y paleta --ds-*.
...
export default function SpringCheck({
  ...
  onChange,
  disabled = false,
  color = "var(--ds-text, #0f2a43)",
  fillColor = "var(--ds-blue, #1465d9)",
  checkColor = "#ffffff",
  boxSize = 18,
  boxRadius = 5,
  fontSize = 14,
  ...
  const gap = label ? Math.min(16, Math.max(8, Math.round(boxSize * 0.43))) : 0;
